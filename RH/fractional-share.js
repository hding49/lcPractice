...
function processOrders(orders, inventory) {
  ...
  for (let [symbol, qty] of inventory) {
    inventoryMap.set(symbol, parseInt(qty));
  }
  ...
  for (let [symbol, type, amount, priceStr] of orders) {
    ...
    if (type === 'B') {
      ...
      if (current >= quantity) {
        ...
        // 不够的话，从交易所买整股来 cover，多的加回库存
        const needed = quantity - current;
        const wholeSharesToBuy = Math.ceil(needed / 100); // 向上取整
        const extra = wholeSharesToBuy * 100 - needed;
        current = extra; // 把多余的部分作为库存
      }
    } else {
      // SELL 订单
      current += quantity;
    ...
    // Flatten 逻辑：如果 >= 100，就卖出整股
    if (current >= 100) {
      current = current % 100;
    }

    inventoryMap.set(symbol, current);
  }
